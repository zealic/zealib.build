    ...
    public class ProcessFile : Task
    {
        ...
        public override bool Execute()
        {
            ...
            foreach (var pipeline in Pipelines)
            {
                ...
                {
                    case "BASE64":
                        list.Add((stream, encode) => CreateBase64Stream(currentPipeline, stream, encode));
                        break;
                    case "GZIP":
                        list.Add((stream, encode) => CreateGZipStream(currentPipeline, stream, encode));
                        break;
                    case "XOR":
                        list.Add((stream, encode) => CreateXorStream(currentPipeline, stream, encode));
                        break;
                    default:
